...
public class KMSpecialOrderActivity extends AppCompatActivity {
    ...
    {
        // String key= OrderDetailsDatabase.push().getKey();
        //  obj.setOrder_id(key);

        //  OrderDetailsDatabase1 =FirebaseDatabase.getInstance().getReference("Orders/"+"-LunCeYj0UgxxynPv8A4"+"/orderPlaced");
        // OrderDishInfoModel g=new OrderDishInfoModel(4,1,"waiting",9,9);
        // OrderDetailsDatabase1.child("2").setValue(g);
        for(int i=0; i< newOrderedDishes.size() ; i++)
        {
            // getDishDetail(newOrderedDishes.get(i).dish_id,obj);
            final int dish_id= newOrderedDishes.get(i).dish_id;
            DishDetailsDatabase.addValueEventListener(new ValueEventListener() {
                @Override
    ...
    public void getDishDetail (final int dish_id , final  OrderModel obj) {
        ...
        DishDetailsDatabase.child("1").setValue(new DishDetail(1,"eggs",2));
        DishDetailsDatabase.child("2").setValue(new DishDetail(1,"mutton",2));
        DishDetailsDatabase.child("3").setValue(new DishDetail(2,"chicken",2));
